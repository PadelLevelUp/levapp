...
7. **One response field (PAD-271, audit M5; number self-assigned, unconfirmed; decided 2026-09-11 (coordinator, owner informed) — the migration `7558c350c002` is held until batch 7, as every migration is while no fresh production dump exists).** The student's answer is one field, `response` (`none` default | `confirmed` | `declined` | `cancelled` | `proactive_decline`), with `responded_at` (UTC) and `recorded_by` (`student` | `coach` | `system` | `import`), written only by the student's own actions (reminder yes/no, cancel, proactive decline) and the import. The coach's record stays `status` / `justification` / `validated` and never moves `response`. `invited` and `confirmed`: phase 1 (migration `7558c350c002`, unconfirmed) keeps them as shadow columns still written where they always were; phase 2 derives them on read (`invited` always true, `confirmed` = `response != none`), moves the readers and both shells to `response`, and drops them. `late_cancellation` is DERIVED, not stored (decided 2026-09-11): it is `response = cancelled` with `responded_at` at or after the cancellation deadline (`attendance.confirm` rule 6); the payload keeps `lateCancellation`, computed on read, and the column is dropped once the backfill has read it. The never-written status values `Lesson.ended` and `LessonInstance.rescheduled` are dropped in the same migration (`NotificationEvent.queued` was on that list until PAD-317 made it a live invitation state, `notifications.invitations` rule 15; it stays), each enum recreated guarded. Backfill from the flags: `status=absent AND validated=false` → `declined` (`cancelled` where `late_cancellation`), `confirmed AND status IS DISTINCT FROM absent` → `confirmed`, else `none`; `responded_at` from the latest reminder attempt where one exists. Stored as a CHECK-constrained string like `enrolment_source`.
   **Phase 2 carries two inherited obligations.** First, and larger: rule 9's `attendanceState` is derived from the legacy shape — `validated` + `status` for the coach's record, then `status == 'absent'` before `confirmed` for the student's intent, that ordering being the whole of B-073. Phase 2 must move it onto `response`, where the five states map directly (`none` → `planned`, `confirmed` → `coming`, `declined` / `cancelled` / `proactive_decline` → `not_coming`, the coach's validated `status` keeping `attended` / `missed`) and the ordering trap disappears, because `response` cannot say *answered* and *coming* with one value. Until then `attendanceState` is the single biggest reader of the flags this rule retires, so it moves first and everything else follows it. It stays author-blind either way: `recorded_by` is what may then answer *who*, and it belongs to the separate provenance fact, never to the state. Second: `attendance.confirm` rule 23's `cancelledByStudent` / `cancelledAt` (PAD-288) are derived from the legacy shape — `status=absent AND justification=justified AND NOT validated`, with the time taken from the row's `updated_at`. That is correct through phase 1, which keeps writing those flags. When phase 2 moves the student paths onto `response`, both fields must be re-derived from `response` (`declined` | `cancelled` | `proactive_decline`) and `responded_at` — which is also the more accurate instant, being the answer's own timestamp rather than the row's last write. Restoring the shadow writes is the tempting fix and the wrong one. `backend/padel_app/tests/test_pad288_early_cancellation.py:57/70/71` pins this end to end through the real `cancel_attendance`, so phase 2 gets a loud failure there rather than a silent `False`; this paragraph is the answer to it.
