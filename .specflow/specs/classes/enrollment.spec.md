...
- **Association_PlayerLessonInstance** (`player_in_lesson_instance`): player_id, lesson_instance_id (unique pair), indexed on lesson_instance_id — PAD-259 phase 1: a shadow copy of `presences`, written by `enrol()` and read by nothing; dropped in phase 2 (`classes.instance-enrollment`)

### Rules
1. Players enrolled at the lesson level appear in ALL future instances
2. Players can also be added to specific instances only — that is a `Presence` row with `enrolment_source=coach` (`classes.instance-enrollment` rule 4, PAD-259)
