...
### Entities
- **LessonInstance** (`lesson_instances`): lesson_id, original_lesson_occurence_date, start_datetime, end_datetime, overwrite_title, level_id, notifications_enabled, status (scheduled|canceled|rescheduled|completed), notes, max_players, overridden_fields (JSON) — indexed on (lesson_id, original_lesson_occurence_date), the occurrence key every materialisation lookup takes (not unique yet: B-033), and on start_datetime
