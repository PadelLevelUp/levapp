...
**The whole outcome is pending the owner (Q2, Q3, Q5, Q6).** Today a player sees no evaluation at
all, so this is a new disclosure. Everything below is the recommended default; nothing is built
...
4. They share. The player gets a message from them in their conversation. *(Q3)*
...
6. The coach can stop sharing. The card disappears from the player's side and nobody is told. *(Q6)*
...
6. Sharing sends one message; updating a share and un-sharing send none. *(Q3)*
7. Deleting an evaluation also stops the player seeing it. *(Q6)*
8. Evaluations recorded before sharing existed can be shared like any other. *(Q5)*
...
- OPEN: owner questions Q2, Q3, Q5, Q6 (`open-questions.md` in the canvas's archive entry).
