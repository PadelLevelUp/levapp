...
- [2026-09-11 — Android push runbook (Firebase/FCM via Expo, EAS), DRAFT](2026-09-11-android-push-firebase-eas.md) — PAD-307 wave C prep: what the code does (channelId/priority, platform validation, optional EXPO_ACCESS_TOKEN), the owner's Firebase/EAS/Play steps in order, decisions (reuse GCP project, enhanced push security before release)
- [2026-09-13 — Rolling production back: the one command and what it does not cover](2026-09-13-prod-rollback-procedure.md) — PAD-338: the VM runs the commit tag, `rollback_to` on deploy-prod.yaml is the one-command way back (skips the build, same secrets), keep-last-three replaces the success-path prune; across a migration there is no rollback — the old entrypoint cannot start — only restore-then-redeploy; the migration window measured at 21 s for 21 migrations (R-035)
...
- [2026-09-11 — Notification engine cost (M17/M18) measured; cheap engine fixes shipped, push and job store for the owner](2026-09-11-notification-engine-cost.md) — DRAFT; 3 stmts/candidate → 13/wave at 300 roster; push blocks the scheduler thread (recommend off-thread sender); job store ~2 s per deploy and per day (recommend keep, no redesign)
...
- [2026-09-10 — The VM takes internet traffic only through nginx; SSH stays open for the deploys](2026-09-10-vm-ingress-nginx-only-ssh-stays-open.md) — PAD-229: every container publishes on 127.0.0.1, the hand-made tcp:5000 and rdp rules go, SSH on 22 stays because the deploys ssh in from GitHub-hosted runners (reaffirmed 2026-09-16, revisit condition written down); the four surviving rules verified in Terraform with a no-change plan
...
- [2026-09-13 — Adopting work we did not write, and what counts as proving a fix](2026-09-13-adopting-unowned-work.md) — an unattended job filed a stale ticket (PAD-312) and a correct PR (#258) within a day of each other; adoption over merge, and the 2×2 as proof of a fix (R-033, R-034)
