...
- **Confirmed in production (2026-09-24, Session-A, read-only nginx log and DB):** the owner's phone was signed in as user 3 but its token was still registered to user 833 (a row from 2026-09-07). Coach 830 messaged conversation 176 (users 830 and 833), and every push tap got a 403. That was the owner's build-24 report.
- **Owner decision D137 (same PR):** rule 9 is rewritten to "the latest login owns the phone". Registration deletes every OTHER user's row for that token, consciously superseding PAD-269, with the trade-off recorded in the rule. The app re-registers on every launch and every login (\`AuthContext.tsx\` :79/:119/:142), so stuck phones heal themselves after the deploy; Session-A also cleans the existing duplicates in production.
  - Tests: \`test_native_push.py\` rewritten. "A second user registering the same token removes the first user's row" and "…leaves the previous user's other devices alone" were both red first; the second was also proven by a mutant that drops the token filter.
  - The 403 on a push-opened thread now reads "This conversation belongs to another account" (cause-neutral copy, with no Retry on iOS, after Session-B's review) on iOS and web, through the shared \`threadLoadErrorKey\` (3/3, red first). The web thread previously showed nothing at all on a failed load.
- **Superseded:** the note below about stuck phones needing an owner cleanup is overtaken by D137.
