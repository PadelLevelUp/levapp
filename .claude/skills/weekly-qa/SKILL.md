...
- `qa/journeys.yaml` — the canonical user journeys to walk (web + iOS, per role).
- `qa/design-rubric.md` — the fixed design-scoring rubric and severity mapping.
- `qa/baseline/baseline-latest.json` — last run's results, for diffing. Absent on the very first (calibration) run.
...
2. Reset + seed the isolated QA database:
   ```bash
   bash /Users/pedropacheco1/Documents/Projetos/padel_app/levelup/qa/scripts/reset-qa-db.sh
   ```
3. Boot the QA backend (from `levelup_backend`, `.venv` active):
   ```bash
   cd /Users/pedropacheco1/Documents/Projetos/padel_app/levelup/levelup_backend
...
4. Boot the web frontend against QA:
   ```bash
   cd /Users/pedropacheco1/Documents/Projetos/padel_app/levelup/levelup_frontend/apps/web
...
5. Boot the iOS simulator against QA (iPhone 17 Pro, per mobile E2E convention):
   ```bash
   cd /Users/pedropacheco1/Documents/Projetos/padel_app/levelup/levelup_frontend/apps/mobile
...
6. Run the web Playwright suite and the iOS Maestro workspace, capturing pass/fail per spec/flow.
   ...
   bash scripts/e2e.sh
   ```
7. Diff results against `qa/baseline/baseline-latest.json`. A spec that **passed last week and fails now** is a P1 regression candidate. Pre-existing failures (see the known-flaky list in `qa/baseline/known-failures.md`) are NOT regressions — do not file them.

### Phase 2 — Exploratory walk (agent-as-user, parallel)

8. For each journey in `qa/journeys.yaml`, spawn a subagent that drives the **real running app** (web → Chrome MCP against `http://localhost:8080`; iOS → computer-use against the Simulator). Fan them out in parallel, grouped by platform (web journeys can run concurrently; iOS journeys are serial — one simulator).
...
11. Spawn a subagent per screen that scores it against `qa/design-rubric.md` and emits the rubric's machine-readable summary block (scores per dimension, findings[], worst_severity). Include the **web↔iOS parity** pass: compare the same feature across platforms for visual/label consistency.

### Phase 4 — Report + triage

12. Merge Phase 1–3 outputs into one dated report at `qa/reports/YYYY-MM-DD.md` (pass a timestamp in — do not call Date.now()). Sections: **Regressions │ Functional bugs │ Design findings**, each severity-ranked, with inline screenshot references. Include a week-over-week delta vs baseline (new / fixed / persisting).
...
14. Update the baseline: write this run's results to `qa/baseline/baseline-latest.json` (regression pass/fail map + per-screen design scores) so next week diffs against it.
