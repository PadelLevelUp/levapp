...
`global-setup` prints the stack it resolved and **refuses to start** — without touching any database — when the chosen backend port is already listening, or when the target is `levelup_test` and something listens on 5001 (another session's live run). `reset-test-db.sh` carries the same `levelup_test`/5001 guard *before* it terminates connections, so a bare `npm run test:e2e:reset` cannot cut a peer's run either. Never kill processes by name (`pkill -f playwright|vite|flask`) to "clean up": other sessions' suites share this machine — kill only PIDs you started, or `lsof -ti :<your port>`.
