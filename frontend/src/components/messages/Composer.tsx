    ...
    <div className="p-4 border-t border-border bg-card">
        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="flex-1 pr-3 border-r-2 border-primary text-right">
            <p className="text-xs font-semibold text-primary">
              {Number(replyingTo.senderId) === Number(userId) ? 'You' : participantName}
            </p>
            <p className="text-xs text-muted-foreground line-clamp-1">{replyingTo.content}</p>
          </div>
          <button onClick={onCancelReply} className="p-1 rounded-full hover:bg-accent" aria-label="Cancel reply">
      ...
      {editingMessage && (
        <div className="flex items-center gap-2 mb-2">
          ...
          <button onClick={() => { onCancelEdit(); setText(''); }} className="p-1 rounded-full hover:bg-accent" aria-label="Cancel edit">
            <X className="h-4 w-4 text-muted-foreground" />
          </button>
        </div>
      )}
