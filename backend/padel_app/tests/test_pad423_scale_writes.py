...
- Every entry stores the scale it was given on, whoever writes it (the records API, the frozen
  ...
  never rewrites it; re-scoring an entry takes the current scale.
...
def test_an_entry_keeps_the_scale_it_was_given_on_and_a_rescore_takes_the_current_one(app):
    ...
    with app.app_context():
        ...
        entry.score = 7.0
        db.session.commit()
        assert (entry.scale_min, entry.scale_max) == (1, 10)
